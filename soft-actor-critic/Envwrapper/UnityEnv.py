...
class UnityWrapper:
    def __init__(
        ...
        if continus:
            self.observation_space, _, self.action_space = self.get_size()
        else:
            self.observation_space, self.action_space, _ = self.get_size()
    ...
    def get_size(self):
        ...
        return (
            ...
            Box(-1, 1, (continuous_action_size,)),
        )
    ...
    def step(self, c_action):
        return self.tstep(None, np.array(c_action).reshape(1, 2))

    def tstep(self, d_action, c_action):
        """
        ...
        self._env.set_actions(
            self.bahavior_name, ActionTuple(continuous=c_action, discrete=d_action)
        )
        self._env.step()
        ...
        return (
            [obs.astype(np.float32) for obs in decision_steps.obs],
...
if __name__ == "__main__":
    ...
    obs_shape_list, d_action_size, c_action_size = env.get_size()
